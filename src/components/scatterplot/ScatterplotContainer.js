...
function ScatterplotContainer({scatterplotData, xAttribute, yAttribute, selectedItems, isFiltered, scatterplotControllerMethods}){
    ...
    useEffect(()=>{
        ...
        const handleSelection = function(selectedData) {
            console.log("handleSelection ...", selectedData);
            // If it's a single item (click) or multiple items (brush), handle accordingly
            scatterplotControllerMethods.updateSelectedItems(Array.isArray(selectedData) ? selectedData : [selectedData]);
        }
        ...
        const controllerMethods = {
            handleOnClick: handleSelection, // Use the same handler for both click and brush
            handleOnMouseEnter,
            handleOnMouseLeave
        }
    ...
    useEffect(()=>{
        ...
        const scatterplotD3 = scatterplotD3Ref.current
        // call renderScatterplot of ScatterplotD3...;
        scatterplotD3.highlightSelectedItems(selectedItems, isFiltered)
    },[selectedItems, isFiltered])
    return(
